...
export async function POST(req:Request){
  const {clientId,reference,comment}=await req.json(); const c=getClient(clientId);
  ...
  c.paymentMethod='BINANCE'; c.paymentReference=reference; c.paymentComment=comment||`TURNAVIA - ${c.name}`; c.status='REVISION_BINANCE';
