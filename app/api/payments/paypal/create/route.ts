...
async function token(){
 ...
 return {access:j.access_token,base};
}
...
export async function POST(req:Request){
 try{
  ...
  const r=await fetch(`${base}/v2/checkout/orders`,{
    ...
    body:JSON.stringify({
      ...
      payment_source:{
        paypal:{
          experience_context:{
            return_url:`${appUrl}/pago/paypal/retorno?client=${encodeURIComponent(c.id)}`,
            cancel_url:`${appUrl}/pago?client=${encodeURIComponent(c.id)}&cancel=1`,
            user_action:'PAY_NOW'
          }
        }
      }
  ...
  if(!r.ok)return NextResponse.json({error:'PayPal no pudo crear la orden',details:data},{status:502});
  c.paymentMethod='PAYPAL';
  c.paypalOrderId=data.id;
  ...
  if(e.message==='PAYPAL_NOT_CONFIGURED')return NextResponse.json({error:'PayPal todavía no tiene credenciales configuradas en .env.local',code:'PAYPAL_NOT_CONFIGURED'},{status:503});
  return NextResponse.json({error:'Error iniciando PayPal',details:e.message},{status:500});
 }
}
