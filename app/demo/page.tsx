   ...
   {view==='professional'&&<>
    ...
    <div className="panel-grid" style={{marginTop:18}}>
      <section className="panel">
        ...
        <p className="muted">Así el profesional controla en qué días y horas puede recibir reservas.</p>
        <div className="grid-3" style={{marginTop:12}}>{(biz.availability||[]).map((a,i)=><div className="card" key={i}><Clock3 size={18}/><strong>{a.days.map(d=>dayNames[d]).join(', ')}</strong><p>{a.start} – {a.end}</p></div>)}</div>
      </section>
      <section className="panel">
        <h2>Catálogo de servicios</h2>
        <p className="muted">Cada servicio tiene su propia duración y precio.</p>
        <div style={{display:'grid',gap:8,marginTop:12}}>{biz.services.map(s=><div className="notice row space" key={s.name}><div><strong>{s.name}</strong><div className="muted" style={{fontSize:12}}>{s.duration} minutos</div></div><strong>{'
