...
export default async function Master(){
  ...
  const clients=rows.map((r:any)=>({
    id:String(r.id),name:r.name||'Sin nombre',type:r.type||'—',specialty:r.specialty||'',phone:r.phone||'—',email:r.email||'—',
  ...
  const mrr=doctors*15+clinics*49;
  ...
  return <div className="dashboard">
    ...
    <main className="main">
      ...
      <div className="stat-grid">
        <div className="stat"><Building2 size={18}/><small style={{display:'block',marginTop:8}}>Clínicas activas</small><div className="n">{clinics}</div></div>
        <div className="stat"><HeartPulse size={18}/><small style={{display:'block',marginTop:8}}>Médicos activos</small><div className="n">{doctors}</div></div>
      ...
      <section className="panel">
        ...
        {clients.length===0 ? <div className="notice" style={{marginTop:16}}>Aún no hay médicos o clínicas registrados como clientes.</div> :
