...
export default async function SuscripcionesMaster({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <div className="dashboard"><Sidebar role="master"/><main className="main">
  <div className="topbar"><div><div className="muted" style={{fontSize:13}}>Cobros TUCITA</div><h1>Suscripciones</h1><div className="muted" style={{fontSize:13}}>Aquí aparecen solo cuentas que ya enviaron un pago o tienen historial de suscripción. Los 15 días de prueba se controlan en Profesionales.</div></div></div>
  <div className="stat-grid">
    <div className="stat"><small>Activos reales</small><div className="n">{active}</div></div>
    <div className="stat"><small>Con historial de pago</small><div className="n">{real.length}</div></div>
  ...
  <section className="panel" style={{marginTop:18}}>
    <form method="get" className="row" style={{gap:10,flexWrap:'wrap',alignItems:'end'}}>
      ...
      <div className="field" style={{minWidth:220}}><label>Estado</label><select name="status" defaultValue={status}><option value="TODOS">Todos</option><option value="ACTIVO">Activo</option><option value="REVISION_BINANCE">Pago en revisión</option><option value="PAGO_PENDIENTE">Pago pendiente</option><option value="SUSPENDIDO">Suspendido</option></select></div>
  ...
  <section className="panel" style={{marginTop:18}}>
    {filtered.length===0?<div className="notice">No hay suscripciones con esos filtros.</div>:<div style={{overflowX:'auto'}}><table className="table"><thead><tr><th>Cliente</th><th>Plan</th><th>Estado</th><th>Ciclo / vigencia</th><th>Pago</th><th>Referencia</th><th>Acción</th></tr></thead><tbody>{filtered.map((r:any)=>{
      ...
      <td>{r.status==='ACTIVO'&&(r.latest_approval?.paidUntil||r.payment_reviewed_at)?<>Vence {new Date(r.latest_approval?.paidUntil||new Date(new Date(r.payment_reviewed_at).getTime()+31*86400000)).toLocaleDateString('es-VE')}</>:r.status==='REVISION_BINANCE'?'Esperando aprobación':r.latest_approval?.paidUntil?<>Último período hasta {new Date(r.latest_approval.paidUntil).toLocaleDateString('es-VE')}</>:'—'}</td>
