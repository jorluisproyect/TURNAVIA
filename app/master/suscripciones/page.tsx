...
export default async function SuscripcionesMaster({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const rows=sql?await sql`SELECT id,name,email,type,status,trial_ends_at,payment_method,payment_reference,payment_submitted_at,payment_reviewed_at,payment_rejection_reason,
   (payment_proof IS NOT NULL) AS has_proof
   FROM commercial_clients ORDER BY created_at DESC`:[];
 ...
 return <div className="dashboard"><Sidebar role="master"/><main className="main">
  ...
  <div className="stat-grid">
    ...
    <div className="stat"><small>MRR real</small><div className="n">${mrr}</div><small>{pending} pendientes · demos excluidos</small></div>
  </div>
