...
export default function EquipoPage(){
  ...
  async function action(body:any){
    ...
    savingRef.current=true;setSaving(true);setMsg('Guardando cambios…');
    try{
      const r=await fetch('/api/me/team',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});
      const j=await r.json();
      if(!r.ok){setMsg(j.error||'No se pudieron guardar los cambios.');return false}
      setMsg('Cambios guardados correctamente.');await load();setTimeout(()=>setMsg(''),2200);return true;
    }catch{
      setMsg('No se pudo conectar con TUCITA. Intenta nuevamente.');return false;
    }finally{
  ...
  function pricePreview(){
    ...
    const targets=service.currency==='VES'?['USD','EUR','USDT']:service.currency==='USD'?['VES','EUR','USDT']:['USD','VES','EUR'].filter(x=>x!==service.currency);
    const label=(cur:string)=>cur==='VES'?'Bs':cur;
    return <div className="live-fx" aria-live="polite"><span>≈</span>{targets.map(cur=>{const v=convert(service.price,service.currency,cur);return <span key={cur}><strong>{v===null?'—':new Intl.NumberFormat('es-VE',{maximumFractionDigits:2}).format(v)}</strong> {label(cur)}</span>})}<small>referencia automática</small></div>;
  }
  ...
  return <div className="dashboard"><Sidebar role="medico"/><main className="main">
    ...
    <div className="panel-grid" style={{marginTop:18}}>
      ...
      <section className="panel">
        ...
        <div className="form">
          ...
          <button className="btn btn-primary" disabled={saving||data.team.length>=data.maxProfessionals} onClick={addMember}><Plus size={16}/> {saving?'Guardando…':'Agregar al equipo'}</button>
        </div>
      </section>
    </div>
    ...
    {current&&<>
      <section className="panel" style={{marginTop:18}}>
        <div className="row space" style={{gap:12,flexWrap:'wrap'}}>
          ...
          <div className="button-row"><button className="btn btn-secondary" onClick={()=>openEditMember(current)}><Pencil size={16}/> Editar ficha</button><button className="btn btn-secondary" onClick={()=>copy(location.origin+'/reservar/'+current.slug)}><Copy size={16}/> Copiar reserva</button>{!current.isOwner&&<button className="btn btn-secondary" disabled={saving} onClick={()=>action({action:'remove_member',doctorId:current.id})}><Trash2 size={16}/> {saving?'Procesando…':'Desactivar'}</button>}</div>
        </div>
      </section>
      ...
      <div className="panel-grid" style={{marginTop:18}}>
        ...
        <section className="panel">
          ...
          <div className="form">
            ...
            <button className="btn btn-primary" onClick={addAvailability} disabled={saving}><CalendarPlus size={16}/> {saving?'Publicando…':'Publicar horario'}</button>
          </div>
        </section>
      </div>
      ...
      <section className="panel" style={{marginTop:18}}>
        ...
        {!current.appointments?.length?<div className="notice" style={{marginTop:12}}>Este profesional todavía no tiene reservas próximas.</div>:<div style={{overflowX:'auto',marginTop:12}}><table className="table"><thead><tr><th>Cliente</th><th>Servicio</th><th>Fecha</th><th>Pago</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{current.appointments.map(a=><tr key={a.id}>
          ...
          <td><div className="row" style={{gap:6,flexWrap:'wrap'}}>
            {a.status==='PAYMENT_REVIEW'&&<><button className="btn btn-primary" disabled={saving} onClick={()=>action({action:'approve_payment',doctorId:current.id,appointmentId:a.id})}>{saving?'Procesando…':'Aprobar'}</button><button className="btn btn-secondary" disabled={saving} onClick={()=>action({action:'reject_payment',doctorId:current.id,appointmentId:a.id})}>{saving?'Procesando…':'Rechazar'}</button></>}
            {a.status==='CONFIRMED'&&<button className="btn btn-secondary" disabled={saving} onClick={()=>action({action:'appointment_status',doctorId:current.id,appointmentId:a.id,status:'ARRIVED'})}>{saving?'Procesando…':'Llegó'}</button>}
            {a.status==='ARRIVED'&&<button className="btn btn-secondary" disabled={saving} onClick={()=>action({action:'appointment_status',doctorId:current.id,appointmentId:a.id,status:'IN_CONSULTATION'})}>{saving?'Procesando…':'Atender'}</button>}
            {a.status==='IN_CONSULTATION'&&<button className="btn btn-primary" disabled={saving} onClick={()=>action({action:'appointment_status',doctorId:current.id,appointmentId:a.id,status:'COMPLETED'})}>{saving?'Procesando…':'Completar'}</button>}
          </div></td>
        </tr>)}</tbody></table></div>}
      </section>
    </>}
    ...
    </div></div><div className="button-row profile-modal-actions"><button className="btn btn-primary" onClick={saveMember} disabled={saving}>{saving?'Guardando…':'Guardar ficha'}</button><button className="btn btn-secondary" onClick={()=>setEditMember(null)}>Cancelar</button></div></div></div>}
