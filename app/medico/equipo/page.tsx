...
:cur==='EUR'?'€':cur==='VES'?'Bs':'USDT';
    return <div className="live-fx" aria-live="polite"><span>≈</span>{targets.map(cur=>{const v=convert(service.price,service.currency,cur);return <span key={cur}><strong>{v===null?'—':new Intl.NumberFormat('es-VE',{maximumFractionDigits:2}).format(v)}</strong> {symbol(cur)}</span>})}<small>referencia automática</small></div>;
  }
