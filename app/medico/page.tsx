...
export default function Medico(){
 ...
 function fxPreview(amount:number,currency:string){
   ...
   const symbol=(cur:string)=>cur==='USD'?'
 function newService(){
...
:cur==='EUR'?'€':cur==='VES'?'Bs':'USDT';
   return <div className="live-fx" aria-live="polite"><span>≈</span>{targets.map(cur=>{const v=converted(amount,currency,cur);return <span key={cur}><strong>{v===null?'—':new Intl.NumberFormat('es-VE',{maximumFractionDigits:cur==='VES'?2:2}).format(v)}</strong> {symbol(cur)}</span>})}<small>referencia automática</small></div>;
 }
 function newService(){
   setService({id:'',name:'',description:'',durationMinutes:30,price:0,currency:'USD'});
   setModal('service');
