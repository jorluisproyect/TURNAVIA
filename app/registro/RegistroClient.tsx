...
export default function RegistroClient(){
  ...
  const initialRole=team||sp.get('role')==='PATIENT'?'PATIENT':'PROFESSIONAL';
  ...
  return <main className="demo-chooser"><div className="container booking-wrap">
    ...
    <div className="demo-head"><span className="eyebrow">Cuenta segura</span><h1>{team?'Cuenta de equipo TUCITA':'Crea tu cuenta TUCITA'}</h1><p className="muted">{team?'Usa el correo al que enviamos la invitación. Tendrás acceso a tu espacio de colaboración, no a datos privados de clientes.':'El nombre que escribas será el nombre real que verá tu panel. Podrás modificarlo desde tu perfil.'}</p></div>
    <section className="profile-card"><form action={action} className="form"><input type="hidden" name="teamInvite" value={team?sp.get('invite')||'':''}/>{team&&<input type="hidden" name="accountType" value="PATIENT"/>}
      {!team&&<div className="field"><label>Tipo de cuenta</label><select name="accountType" value={accountType} onChange={e=>setAccountType(e.target.value)}>
      ...
      {team&&<div className="notice">Tu acceso será <strong>Master colaborador</strong>, con permisos limitados a tu rol técnico.</div>}{state?.error&&<div className="notice danger">{state.error}</div>}
      <button className="btn btn-primary" disabled={pending}>{pending?'Creando cuenta...':team?'Unirme al equipo':'Crear mi cuenta'}</button>
