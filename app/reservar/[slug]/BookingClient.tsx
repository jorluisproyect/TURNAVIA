...
type Service={id:string;name:string;description:string;summary?:string;travelImage?:string;travelDate?:string;departureTime?:string;returnTime?:string;locationId?:string;capacity?:number;durationMinutes:number;price:number;currency:string};
...
export default function BookingClient({slug}:{slug:string}){
 ...
 return <div className="booking"><div className="container booking-wrap">
   ...
   <section className="profile-card">
       ...
       {travelMode
         ?<div className="travel-service-grid">{data.services.map(s=><button type="button" key={s.id} className={'travel-service-card '+(serviceId===s.id?'selected':'')} onClick={()=>setServiceId(s.id)}>
            ...
            <div className="travel-service-content">
              ...
              <div className="muted" style={{fontSize:11}}>Duración {s.durationMinutes} min · {s.capacity||1} cupo{Number(s.capacity||1)===1?'':'s'} · {serviceId===s.id?'Seleccionado':'Toca para elegir'}</div>
            </div>
          </button>)}</div>
     ...
     {travelMode
       ?<div className="notice" style={{marginTop:20,border:'2px solid #0f766e'}}>
          ...
          <span className="muted" style={{fontSize:12}}>
            {startsAt
              ?((selectedBlock?.slots?.find(s=>s.startsAt===startsAt)?.remaining??0)+' cupo'+((selectedBlock?.slots?.find(s=>s.startsAt===startsAt)?.remaining??0)===1?'':'s')+' disponible'+((selectedBlock?.slots?.find(s=>s.startsAt===startsAt)?.remaining??0)===1?'':'s'))
              :'Esta salida no tiene cupos disponibles en este momento.'}
          </span>
        </div>
       :<>
