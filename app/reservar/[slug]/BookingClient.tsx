...
export default function BookingClient({slug,patientLoggedIn=false}:{slug:string;patientLoggedIn?:boolean}){
 ...
 return <div className="booking"><div className="container booking-wrap">
   ...
   <section className="profile-card">
     ...
     {usesProfessionalCredentials(provider.category,provider.activity)&&provider.credentialStatus==='APPROVED'&&<div className="notice" style={{marginTop:14,borderColor:'#86d4bf',background:'#eefaf6'}}><strong>✓ Certificaciones revisadas por TUCITA</strong>{provider.licenseNumber?<><br/>{provider.licenseNumber}</>:null}<br/><small>Revisión documental interna. No sustituye la habilitación oficial del colegio, gremio u organismo competente.</small></div>}
     {(provider.workImages||[]).length>0&&(!usesProfessionalCredentials(provider.category,provider.activity)||provider.credentialStatus==='APPROVED')&&<div style={{marginTop:16}}><div className="muted" style={{fontSize:12,marginBottom:8}}>{usesProfessionalCredentials(provider.category,provider.activity)?'Certificaciones aprobadas':'Referencias de trabajos'}</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(110px,1fr))',gap:8}}>{(provider.workImages||[]).map((img,i)=><img key={i} src={img} alt={(usesProfessionalCredentials(provider.category,provider.activity)?'Credencial ':'Referencia ')+(i+1)} style={{width:'100%',height:92,borderRadius:14,objectFit:'cover'}}/>)}</div></div>}
