...
export function PaymentMethodsManager({scope,slug='sofia-mendoza'}:{scope:'MASTER'|'DOCTOR';slug?:string}){
  ...
  return <section className="panel" style={{marginTop:18}}>
    <div className="row space"><div><h2>Métodos de pago</h2><div className="muted" style={{fontSize:13}}>{scope==='MASTER'?'Cómo pagan médicos y clínicas a TURNAVIA.':'Solo los métodos activos se muestran a tus pacientes.'}</div></div><button className="btn btn-primary" onClick={()=>setShow(!show)}><Plus size={16}/> Agregar método</button></div>
